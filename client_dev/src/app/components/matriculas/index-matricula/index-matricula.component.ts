...
export class IndexMatriculaComponent implements OnInit {
  ...
  init_matriculas_hoy() {
    ...
    this._matriculaService
      .obtener_matriculas_hoy(this.token)
      .subscribe((response) => {
  ...
  buscador_avanzado() {
    if (this.tipo_filtro) {
      if (this.tipo_filtro == 'codigo') {
        ...
        this.matriculas = this.matriculas_const.filter((item) =>
          term.test(item._id)
        );
      } else if (this.tipo_filtro == 'nombres') {
        let term = new RegExp(this.filtro_valor, 'i');
        this.matriculas = this.matriculas_const.filter(
          (item) =>
            term.test(item.clinete.nombres) ||
            term.test(item.clinete.apellidos) ||
            term.test(item.clinete._id)
        );
      } else if (this.tipo_filtro == 'canal') {
        let term = new RegExp(this.filtro_valor, 'i');
        this.matriculas = this.matriculas_const.filter((item) =>
          term.test(item.canal)
        );
      }
    } else {
  ...
  generar_token(matricula: any, cliente: any) {
    this._clienteService
      .generar_token_encuesta_admin(matricula, cliente, this.token)
      .subscribe((response) => {
  ...
  init_comentarios(id: any) {
    ...
    this._matriculaService
      .obtener_comentarios_matricula_admin(id, this.token)
      .subscribe((response) => {
