...
@Component({
  selector: 'app-correos-prospecto',
  templateUrl: './correos-prospecto.component.html',
  styleUrls: ['./correos-prospecto.component.css']
})
export class CorreosProspectoComponent implements OnInit {
    ...
    if (!this.correo.asunto) {
      ...
      this._clienteService.crear_correo_prospeccion_admin(this.correo, this.token).subscribe(
        response => {
          ...
          this.init_data();
          this.correo = {};
          $.notify('Se envio el correo al prospecto', {
            type: 'success',
            spacing: 10,
            timer: 2000,
            placement: {
