...
@Component({
  selector: 'app-edit-prospecto',
  templateUrl: './edit-prospecto.component.html',
  styleUrls: ['./edit-prospecto.component.css']
})
export class EditProspectoComponent implements OnInit {
      ...
      this._clienteService.editar_cliente_admin(this.id, this.cliente, this.token).subscribe(
        response => {
          if (response.data == undefined) {
            ...
            this.btn_actualizar = false;
            $.notify('Se actualizó el prospecto.', {
              type: 'success',
              spacing: 10,
              timer: 2000,
              placement: {
