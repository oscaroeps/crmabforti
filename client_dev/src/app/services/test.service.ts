 ...
 */
  ...
  isAuthenticate() {
    ...
    try {
      ...
      //VALIDAMOS SI TENEMOS UN TOKEN EN EL LOCALSTORAGE
      if (!token) {
        localStorage.clear();
        return false;
      }

      //VALIDAMOS SI EL TOKEN ES INCORRECTO ES DECIR NO CUMPLE CON EL ESTANDAR DE LAS TRES PARTES
      if (!decodedToken) { //header, payload, firma
        localStorage.clear();
        return false;
      }
