...
const listar_llamadas_prospeccion_admin = async function (req, res) {
    if (req.user) {
        ...
        let llamadas = await Cliente_llamada.find({ cliente: id }).populate('asesor').sort({ createAt: -1 });
...
const crear_tarea_prospeccion_admin = async function (req, res) {
    if (req.user) {
        ...
        crear_actividad_prospeccion_admin(
            'Tarea',
            tarea.asesor,
            'Se registró una tarea como ' + tarea.tarea,
            tarea.cliente
        );
...
const listar_tareas_prospeccion_admin = async function (req, res) {
    if (req.user) {
        ...
        let tareas = await Cliente_tarea.find({ cliente: id }).populate('asesor').populate('marca_asesor').sort({ createAt: -1 });
...
const listar_actividades_prospeccion_admin = async function (req, res) {
    if (req.user) {
        ...
        let actividades = await Cliente_actividad.find({ cliente: id }).populate('asesor').sort({ createAt: -1 });
