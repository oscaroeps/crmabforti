    ...
    if (req.user) {
        ...
        try {
            ...
            if (productos.length >= 1) {
                res.status(200).send({ data: undefined, message: 'Ya existe un producto con ese título.' });
            } else {
                //
            ...
            console.log(error);
            res.status(200).send({ data: undefined, message: 'Ocurrió un problema al registrar el producto.' });
        }
    } else {
...
const registrar_inventario_admin = async function (req, res) {
    if (req.user) {
        ...
        let margen_ganancia = data.ganancia_producto; //30
        ...
        if (producto.precio == 0) {
            ...
            //
            let ganancia_actual = producto.stock * producto.costo_unidad;
...
const obtener_inventario_admin = async function (req, res) {
    if (req.user) {
        //greter than equal => mayor igual que
