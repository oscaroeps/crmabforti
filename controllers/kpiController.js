...
const kpi_pagos_mensuales = async function (req, res) {
    if (req.user) {
        ...
        for (var item of pagos) {
            let element_date = new Date(item.createAt);
    ...
    if (req.user) {
        ...
        for (var item of cursos) {
            ...
            /* for (var subitem of reg_pagos) {
                if (item == subitem.matricula_detalle.curso.titulo) { Existe un error y se resolvió con Gemini, tomar en cuenta que este es el código del curso pero se reemplaza con el nuevo que toma en cuenta los registros undefined
                    monto = monto + subitem.monto;
                }
            } */
            for (var subitem of reg_pagos) {
                if (
                    subitem.matricula_detalle &&
                    subitem.matricula_detalle.curso &&
                    item == subitem.matricula_detalle.curso.titulo
                ) {
                    monto = monto + subitem.monto;
                }
            }
